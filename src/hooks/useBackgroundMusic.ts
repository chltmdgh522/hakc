...
export const useBackgroundMusic = (options: UseBackgroundMusicOptions) => {
  ...
  useEffect(() => {
    console.log('useBackgroundMusic: useEffect 실행', { src, loop, autoplay });
    // 오디오 요소 생성
  ...
  const play = async () => {
    console.log('useBackgroundMusic: play 호출됨', { audioRef: !!audioRef.current, isPlaying });
    if (audioRef.current && !isPlaying) {
      ...
      console.log('useBackgroundMusic: 재생 조건 불만족', { audioRef: !!audioRef.current, isPlaying });
    }
  };
